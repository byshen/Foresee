...
def main(exp, M, N):
    all = np.loadtxt('%s/%s_norm_train' % (exp,exp))
    ...
    # add output layer
    #2 = add_layer(l1,10,10,activation_function=tf.nn.relu)

    prediction = add_layer(l1, 10, N+1-M, activation_function=None)
    ...
    test = np.loadtxt('%s/%s_norm_test'%(exp,exp))
